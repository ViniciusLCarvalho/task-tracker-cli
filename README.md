...
Solução para o desafio do [task-tracker](https://roadmap.sh/projects/task-tracker) do [roadmap.sh](https://roadmap.sh/).
...
```powershell
java -jar target/task-tracker-cli.jar <comando>
```

Por exemplo:

```powershell
java -jar target/task-tracker-cli.jar add "Estudar Java"
```
...
| --- | --- | --- |
| `add "descrição"` | Cria uma tarefa com status `todo`. | `java -jar target/task-tracker-cli.jar add "Comprar leite"` |
| `update id "descrição"` | Altera a descrição da tarefa. | `java -jar target/task-tracker-cli.jar update 1 "Comprar leite e pão"` |
| `delete id` | Remove uma tarefa. | `java -jar target/task-tracker-cli.jar delete 1` |
| `list` | Exibe todas as tarefas. | `java -jar target/task-tracker-cli.jar list` |
| `list todo` | Exibe apenas tarefas pendentes. | `java -jar target/task-tracker-cli.jar list todo` |
| `list in-progress` | Exibe apenas tarefas em andamento. | `java -jar target/task-tracker-cli.jar list in-progress` |
| `list done` | Exibe apenas tarefas concluídas. | `java -jar target/task-tracker-cli.jar list done` |
| `mark-in-progress id` | Marca uma tarefa como em andamento. | `java -jar target/task-tracker-cli.jar mark-in-progress 1` |
| `mark-done id` | Marca uma tarefa como concluída. | `java -jar target/task-tracker-cli.jar mark-done 1` |
...
```powershell
# Criar uma tarefa
java -jar target/task-tracker-cli.jar add "Preparar apresentação"
